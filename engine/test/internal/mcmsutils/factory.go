...
// GetConverterFactory returns a blockchain-specific ConverterFactory based on the provided blockchain type.
func GetConverterFactory(
	blockchain fchain.BlockChain,
) (ConverterFactory, error) {
	switch c := blockchain.(type) {
	case fchainevm.Chain:
...
func GetExecutorFactory(
	...
	case fchainevm.Chain:
		...
		if !ok {
			return nil, fmt.Errorf("encoder not found for chain %d", c.Selector)
		}
