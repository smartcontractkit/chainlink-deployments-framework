...
type State struct {
	...
	Proposals   map[string][]string                    // All MCMS and timelock proposals keyed by task id
...
func seedStateFromEnvironment(e fdeployment.Environment) *State {
	return &State{
		...
		Outputs:     make(map[string]fdeployment.ChangesetOutput),
		Proposals:   make(map[string][]string, 0),
	}
}
...
func (s *State) MergeChangesetOutput(id string, out fdeployment.ChangesetOutput) error {
	...
	if err := s.mergeProposals(id, out); err != nil {
...
func (s *State) mergeProposals(id string, out fdeployment.ChangesetOutput) error {
	proposals := make([]string, 0, len(out.MCMSProposals)+len(out.MCMSTimelockProposals))
	...
	for _, p := range out.MCMSProposals {
		...
		proposals = append(proposals, string(b))
	}
