...
func newTestChain(selector uint64) fchain.BlockChain {
	return evm.Chain{
		Selector: selector,
	}
}
...
func TestComponents_AddChains(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			chainsToAdd: func() []fchain.BlockChain { return []fchain.BlockChain{newTestChain(1)} },
			wantCount:   1,
		...
		},
		{
			...
			setupChains: func() []fchain.BlockChain { return []fchain.BlockChain{newTestChain(1)} },
			chainsToAdd: func() []fchain.BlockChain {
				return []fchain.BlockChain{
					newTestChain(2),
					newTestChain(3),
		...
		for i := range numGoroutines {
			go func(goroutineID int) {
				...
				for j := range chainsPerGoroutine {
					chainID := goroutineID*chainsPerGoroutine + j
					chains[j] = newTestChain(uint64(chainID)) //nolint:gosec // G115: this will not overflow from a range
				}
