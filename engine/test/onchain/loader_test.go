...
// Ensure the testChain implements the fchain.BlockChain interface.
var _ fchain.BlockChain = &testChain{}

// testChain is an lightweight implementation of the fchain.BlockChain interface for testing
// purposes.
type testChain struct{ selector uint64 }

func (c *testChain) ChainSelector() uint64 { return c.selector }
func (c *testChain) Family() string        { return "test" }
func (c *testChain) Name() string          { return "test" }
func (c *testChain) String() string        { return fmt.Sprintf("testChain(%d)", c.selector) }
...
func TestLoader_Load(t *testing.T) {
	...
	mockFactory := func(t *testing.T, selector uint64) (fchain.BlockChain, error) {
		t.Helper()
		return &testChain{selector: selector}, nil
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			loader := &ChainLoader{
				selectors: []uint64{csel1, csel2, csel3}, // Provide enough selectors
				factory:   tt.factory,
			}
