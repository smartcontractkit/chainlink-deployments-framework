...
func Test_LoadNetworks(t *testing.T) {
	...
	err = os.WriteFile(
		filepath.Join(configDir, "networks.yaml"), fixture, 0644,
	)
	require.NoError(t, err)
	...
	err = os.WriteFile(
		filepath.Join(streamsConfigDir, "networks.yaml"), fixture, 0644,
	)
	require.NoError(t, err)
	...
	tests := []struct {
		...
		{
			name:       "Staging (data-streams exception)",
			giveEnv:    environment.Staging,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// For filtering behavior we don't need GAP transforms; pass nil Config.
...
func Test_loadNetworkConfig(t *testing.T) {
  ...
  networks:
    - type: "mainnet"
      ...
      - rpc_name: "test_rpc"
        ...
        http_url: "https://rpcs.cldev.sh/ethereum-mainnet"
        ws_url: "wss://rpcs.cldev.sh/ethereum-mainnet"
      - rpc_name: "test_rpc2"
        preferred_url_scheme: "http"
  ...
  `
		...
		RPCs: []cldf_config_network.RPC{
			{
				RPCName:            "test_rpc",
				PreferredURLScheme: "http",
				HTTPURL:            "https://gap-rpc-proxy.public.main.prod.cldev.sh:4443/ethereum-mainnet",
				WSURL:              "wss://gap-rpc-proxy.public.main.prod.cldev.sh:9443/ethereum-mainnet",
			},
			{
				RPCName:            "test_rpc2",
				PreferredURLScheme: "http",
	...
	// Private-only values injected via Config for transformation expectations above.
	injected := &Config{
		RPCsHost:     "rpcs.cldev.sh",
		GapProxyHost: "gap-rpc-proxy.public.main.prod.cldev.sh",
		GapWSPort:    "9443",
		GapHTTPPort:  "4443",
		UseGAP:       true,
	}
...
func Test_gapURLTransform(t *testing.T) {
	...
	const (
		originalHost = "rpcs.cldev.sh"
		proxyHost    = "gap-rpc-proxy.public.main.prod.cldev.sh"
	)
	...
	tests := []struct {
		...
		{
			name: "replaces the http uri host with the GAP V2 proxy address",
			give: "https://rpcs.cldev.sh/ethereum-mainnet",
			port: "4443",
			want: "https://gap-rpc-proxy.public.main.prod.cldev.sh:4443/ethereum-mainnet",
		},
		{
			name: "replaces the websocket uri host with the GAP V2 proxy address",
			give: "wss://rpcs.cldev.sh/ethereum-mainnet",
			port: "9443",
			want: "wss://gap-rpc-proxy.public.main.prod.cldev.sh:9443/ethereum-mainnet",
		},
		{
			name: "leaves uris with non-rpcs.cl-dev.sh URLs unchanged",
			give: "https://test.rpc",
			port: "4443",
			want: "https://test.rpc",
		},
