...
import (
	...
	"github.com/smartcontractkit/chainlink-deployments-framework/engine/cld/legacy/cli/mcmscmd/layout"
)
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if outputFile == "" {
				...
				err := os.WriteFile(outputFile, []byte(analyzedProposal), 0644) //nolint:gosec
				if err != nil {
					return err
				}
			}
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if outputFile == "" {
				...
				err := os.WriteFile(outputFile, []byte(convertedProposal), 0644) //nolint:gosec
				if err != nil {
					return err
				}
			}
