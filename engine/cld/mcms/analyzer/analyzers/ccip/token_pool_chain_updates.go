...
func (a *TokenPoolChainUpdatesAnalyzer) Analyze(
	...
	for _, update := range chainsToAdd {
		...
		if !chainExists {
			annotations = append(annotations, analyzer.NewAnnotation("chain update", "", fmt.Sprintf("%s added", remoteLabel)))
		} else {
			annotations = append(annotations, analyzer.NewAnnotation("warning", "", fmt.Sprintf("%s already enabled", remoteLabel)))
		}
		...
		if update.OutboundRateLimiterConfig.Capacity != nil {
			...
			annotations = append(annotations, compareRateLimiterConfig(
				...
				decimals, fmt.Sprintf("outbound to %s", remoteLabel), tokenSymbol,
			)...)
		}
		...
		if update.InboundRateLimiterConfig.Capacity != nil {
			...
			annotations = append(annotations, compareRateLimiterConfig(
				...
				decimals, fmt.Sprintf("inbound from %s", remoteLabel), tokenSymbol,
			)...)
		}
	}
	...
	for _, sel := range selectorsToRemove {
		...
		if chainExists {
			annotations = append(annotations, analyzer.NewAnnotation("chain update", "", fmt.Sprintf("%s removed", remoteLabel)))
		} else {
			annotations = append(annotations, analyzer.NewAnnotation("warning", "", fmt.Sprintf("%s already disabled", remoteLabel)))
		}
	}
...
func compareRateLimiterConfig(
	...
	if current.IsEnabled != proposed.IsEnabled {
		...
		annotations = append(annotations, analyzer.NewAnnotation("rate limiter", direction, fmt.Sprintf("rate limiter %s", action)))
	}
