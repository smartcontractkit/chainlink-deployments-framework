...
// rawDoc serializes fixtures into the raw stategen document shape.
...
func TestDiff(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "MCMS signer and quorum changes are visible business state",
			before: chainsDoc(
			...
			wantChanges: []changeProjection{
				{path: "chains." + strconv.FormatUint(sel, 10) + ".0xabc.config.groupQuorums[0]", kind: KindChanged},
				{path: "chains." + strconv.FormatUint(sel, 10) + ".0xabc.config.signers[0]", kind: KindChanged},
				{path: "chains." + strconv.FormatUint(sel, 10) + ".0xabc.config.signers[1]", kind: KindRemoved},
			},
		},
		{
			name: "MCMS quorum and sibling changes remain visible across contract types",
			before: chainsDoc(
				view("0xabc", "ManyChainMultiSig", map[string]any{
		...
		{
			name:   "empty before document yields address additions",
		...
		},
		{
			...
			wantChanges: []changeProjection{
				{path: "chains." + strconv.FormatUint(otherSel, 10) + ".0xdef", kind: KindRemoved},
			},
		},
		{
...
// TestDiffReplay regression-locks the untouched historical pinned fixture.
// It spans test-signer surgery: the pure differ must expose all 47 signer
// configuration deltas as well as the five lane changes and 64 unreadables.
// The execution integration owns compensation for that surgery; hiding signer
// changes globally would also hide authored setConfig operations.
func TestDiffReplay(t *testing.T) {
	...
	const (
		...
		mcmAddr         = "0xd9757aa52907798d1af2fda7a6c0cc733e5acf7e"
	)
	...
	wantHarness := make(map[string]ChangeKind)
	harnessPrefix := "chains." + strconv.FormatUint(sel, 10) + "." + mcmAddr + "_CLLCCIP.config."
	for i := range 4 {
		wantHarness[harnessPrefix+"groupQuorums["+strconv.Itoa(i)+"]"] = KindChanged
	}
	wantHarness[harnessPrefix+"signers[0].addr"] = KindChanged
	wantHarness[harnessPrefix+"signers[0].group"] = KindChanged
	for i := 1; i < 42; i++ {
		wantHarness[harnessPrefix+"signers["+strconv.Itoa(i)+"]"] = KindRemoved
	}
	require.Len(t, changes, len(wantEffects)+len(wantHarness))
	...
	for _, c := range changes {
		...
		require.NotContains(t, c.Path, "lastUpdated", "rate-limiter clock noise survived: %s", c.Path)
		if c.Address == mcmAddr {
			kind, expected := wantHarness[c.Path]
			require.True(t, expected, "unexpected MCMS change: %s", c.Path)
			require.Equal(t, kind, c.Kind)
			delete(wantHarness, c.Path)
	...
	require.Empty(t, wantHarness, "expected signer-surgery effects missing from the pure diff")
