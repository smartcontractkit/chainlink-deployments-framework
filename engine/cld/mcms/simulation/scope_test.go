...
func TestResolveScope_EVMTargetsAndContext(t *testing.T) {
	...
	ds := seedDS(t,
		...
		ref(uint64(ethSel), "0xWRAPPED", "WETH9", "1.0.0"),
	)
	...
	require.Len(t, scope.Context, 5)
	contextTypes := make([]string, 0, len(scope.Context))
	for _, r := range scope.Context {
		contextTypes = append(contextTypes, string(r.Type))
	}
	require.ElementsMatch(t, []string{"ProposerManyChainMultiSig", "RBACTimelock", "Router", "ERC20Token", "WETH9"}, contextTypes)
}
...
func TestResolveScope_FixtureEVMMultiTarget(t *testing.T) {
	...
	require.Len(t, scope.Context, 136)
	...
	require.Contains(t, contextTypes(scope), "WETH9")
}
...
func TestResolveScope_FixtureEmptyContractType(t *testing.T) {
	...
	require.Len(t, evmScope.Context, 136)
	require.Contains(t, contextTypes(evmScope), "WETH9")
}
