...
func ResolveScope(
	...
	for _, r := range refs.Filter(datastore.AddressRefByChainSelector(uint64(chainSelector))) {
		byAddr[scopeAddressKey(r.Address)] = r
	}
	...
	// Context: the chain's MCM and timelock for diagnostics, plus every router
	// and token ref needed by dependent views, including wrapped-native WETH9.
	seen := map[string]struct{}{}
	addContext := func(ref datastore.AddressRef) {
		key := scopeAddressKey(ref.Address)
	...
	for _, addr := range []string{meta.MCMAddress, proposal.TimelockAddresses[chainSelector]} {
		if ref, ok := byAddr[scopeAddressKey(addr)]; ok {
			addContext(ref)
		}
	}
	contextAddrs := make([]string, 0, len(byAddr))
	for addr, ref := range byAddr {
		if strings.Contains(string(ref.Type), "Router") || strings.Contains(string(ref.Type), "Token") || ref.Type == "WETH9" {
...
func scopeAddressKey(address string) string {
