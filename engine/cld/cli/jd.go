...
import (
	...
	"github.com/smartcontractkit/chainlink-deployments/pkg/lib/pointer"
	"github.com/smartcontractkit/chainlink-deployments/pkg/offchain"
)
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, name := range dons {
				...
				// https://github.com/smartcontractkit/chainlink-deployments/blob/69fe026a06949da41f2ed250a057a8ae24b47fc6/domains/keystone/shared/changeset_register_nodes_with_jd.go#L41
				donName := "don-" + name
				selectors = append(selectors, &ptypes.Selector{
					Key: donName,
			...
			switch format {
			case "table":
				// TODO(mstreet3): update writeNodeTable to support jobs view
				...
				for key, value := range newLabels {
					...
					if value == "" {
						...
						if index >= 0 {
							...
							labels[index].Value = pointer.To(value)
						} else {
							// add new value
							labels = append(labels, &ptypes.Label{Key: key, Value: pointer.To(value)})
						}
					}
				}
...
func loadOffchainClient(
	...
	cfg, err := environment.LoadEnvConfig(domain, envKey)
	if err != nil {
		return nil, err
	}

	return environment.LoadOffchainClient(ctx, domain, envKey, cfg, lggr, true)
}
