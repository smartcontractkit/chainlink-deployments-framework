...
func (c Commands) NewStateCmds(
	...
	stateCmd.AddCommand(c.newStateGenerate(domain, config))
...
func (c Commands) newStateGenerate(domain cldf_domain.Domain, cfg StateConfig) *cobra.Command {
	var (
		persist       bool
		outputPath    string
	...
	cmd := cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			envCtx, cancel := context.WithTimeout(cmd.Context(), viewTimeout)
			defer cancel()
			env, err := environment.LoadEnvironment(func() context.Context {
				return envCtx
			}, c.lggr, envKey, domain, true /*useRealBackends*/)
			if err != nil {
			...
			if persist {
				...
				if outputPath != "" {
					err = cldf_domain.SaveViewState(outputPath, state)
				} else {
