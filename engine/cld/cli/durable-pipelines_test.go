...
func TestNewDurablePipelineRunCmd(t *testing.T) {
	...
	tempLoadEnv := environment.LoadEnvironment
	...
	loadEnv = func(getCtx func() context.Context, lggr logger.Logger, envName string,
		domain domain.Domain, writeArtifacts bool, options ...environment.LoadEnvironmentOption) (cldf.Environment, error) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			rootCmd := sharedCommands.NewDurablePipelineCmds(
				testDomain,
				func(envName string) (*migrations.MigrationsRegistry, error) {
					rp := migrationsRegistryProviderStub{
						BaseRegistryProvider: migrations.NewBaseRegistryProvider(),
						AddMigrationAction: func(registry *migrations.MigrationsRegistry) {
							registry.Add(changeset, migrations.Configure(&changesetStub).With(1))
						},
					}
				...
				migrations.NewConfigResolverManager(),
			)
...
func TestNewDurablePipelineInputGenerateCmd(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(filepath.Join(workspaceRoot, "go.mod"), []byte("module test"), 0644))
  ...
  - 0001_test_changeset:
	...
	for _, formatTest := range formatTests {
		t.Run(formatTest.name, func(t *testing.T) {
			...
			require.NoError(t, os.WriteFile(inputsFilePath, []byte(formatTest.mockInputContent), 0644))
			...
			for _, tt := range tests {
				t.Run(tt.name, func(t *testing.T) {
					...
					var testResolver migrations.ConfigResolver
					...
					resolverManager := migrations.NewConfigResolverManager()
					resolverManager.Register(testResolver, migrations.ResolverInfo{
					...
					rootCmd := sharedCommands.NewDurablePipelineCmds(
						...
						func(envName string) (*migrations.MigrationsRegistry, error) {
							rp := migrationsRegistryProviderStub{
								BaseRegistryProvider: migrations.NewBaseRegistryProvider(),
								AddMigrationAction: func(registry *migrations.MigrationsRegistry) {
									changeset := &stubChangeset{resolver: testResolver}
									registry.Add("0001_test_changeset", migrations.Configure(changeset).WithConfigResolver(testResolver))
								},
							}
...
func TestBuildListCmd(t *testing.T) {
	...
	tests := []struct {
		name                   string
		args                   []string
		setupMocks             func() (*migrations.MigrationsRegistry, *migrations.ConfigResolverManager, error)
		expectedErr            string
	...
	}{
		{
			...
			setupMocks: func() (*migrations.MigrationsRegistry, *migrations.ConfigResolverManager, error) {
				...
				resolverManager := migrations.NewConfigResolverManager()
				resolverManager.Register(MockTestResolver, migrations.ResolverInfo{
					Description: "Test Resolver",
				...
				rp := migrationsRegistryProviderStub{
					...
					AddMigrationAction: func(registry *migrations.MigrationsRegistry) {
						...
						registry.Add("0001_static_changeset", migrations.Configure(staticChangeset).With(1))

						// Dynamic changeset (with resolver)
						dynamicChangeset := &stubChangeset{resolver: MockTestResolver}
						registry.Add("0002_dynamic_changeset", migrations.Configure(dynamicChangeset).WithConfigResolver(MockTestResolver))

						// Error changeset (resolver not registered with manager)
						errorChangeset := &stubChangeset{resolver: MockUnregisteredResolver}
						registry.Add("0003_error_changeset", migrations.Configure(errorChangeset).WithConfigResolver(MockUnregisteredResolver))
					},
				}
		...
		},
		{
			...
			setupMocks: func() (*migrations.MigrationsRegistry, *migrations.ConfigResolverManager, error) {
				...
				resolverManager.Register(MockFirstResolver, migrations.ResolverInfo{
					Description: "First Resolver",
					ExampleYAML: "test: value1",
				})

				resolverManager.Register(MockSecondResolver, migrations.ResolverInfo{
				...
				rp := migrationsRegistryProviderStub{
					BaseRegistryProvider: migrations.NewBaseRegistryProvider(),
					AddMigrationAction: func(registry *migrations.MigrationsRegistry) {
						changeset := &stubChangeset{resolver: MockFirstResolver}
						registry.Add("0001_test_changeset", migrations.Configure(changeset).WithConfigResolver(MockFirstResolver))
					},
				}
		...
		},
		{
			...
			setupMocks: func() (*migrations.MigrationsRegistry, *migrations.ConfigResolverManager, error) {
				return migrations.NewMigrationsRegistry(), migrations.NewConfigResolverManager(), nil
			},
			expectedErr: "required flag(s) \"environment\" not set",
		},
		{
			...
			setupMocks: func() (*migrations.MigrationsRegistry, *migrations.ConfigResolverManager, error) {
				return nil, migrations.NewConfigResolverManager(), errors.New("registry load failed")
			},
...
func (m *mockProposalContext) GetABIBy(chainSelector uint64, address string) (*abi.ABI, string, error) {
	return &abi.ABI{}, "", nil
}
...
func (m *mockProposalContext) GetSolanaInstructionDecoder(chainSelector uint64, address string) (any, error) {
	return func() {}, nil
}

type migrationsRegistryProviderStub struct {
	*migrations.BaseRegistryProvider
	AddMigrationAction func(registry *migrations.MigrationsRegistry)
}
...
type stubChangeset struct {
	ApplyCalled bool
	StubError   error
	resolver    migrations.ConfigResolver
}
...
`
	require.NoError(t, os.WriteFile(inputsFilePath, []byte(mockInputContent), 0644))
		...
		func(envName string) (*migrations.MigrationsRegistry, error) {
			rp := migrationsRegistryProviderStub{
				...
				AddMigrationAction: func(registry *migrations.MigrationsRegistry) {
					changeset := &stubChangeset{resolver: testResolver}
					registry.Add("0001_test_changeset", migrations.Configure(changeset).WithConfigResolver(testResolver))
					registry.Add("0002_test_changeset", migrations.Configure(changeset).WithConfigResolver(testResolver))
				},
			}
...
changesets:
  test_changeset:
	...
	require.NoError(t, os.WriteFile(yamlFilePath, []byte(yamlContent), 0644))
  ...
  test_changeset:
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			yamlFilePath := filepath.Join(inputsDir, yamlFileName)
			require.NoError(t, os.WriteFile(yamlFilePath, []byte(tt.yamlContent), 0644))
