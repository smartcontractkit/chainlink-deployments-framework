...
var loadEnv = environment.LoadEnvironment
...
func (c Commands) NewDurablePipelineCmds(
	...
	loadMigration func(envName string) (*migrations.MigrationsRegistry, error),
	decodeProposalCtxProvider func(env cldf.Environment) (mcms.ProposalContext, error),
	loadConfigResolvers *migrations.ConfigResolverManager) *cobra.Command {
...
func (c Commands) newDurablePipelineRun(
	...
	loadMigration func(envName string) (*migrations.MigrationsRegistry, error),
	decodeProposalCtxProvider func(env cldf.Environment) (mcms.ProposalContext, error),
	loadConfigResolvers *migrations.ConfigResolverManager,
) *cobra.Command {
	var (
		changesetStr string
		migrationStr string // deprecated
		dryRun       bool
		inputFile    string
	)
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Handle deprecated --migration flag (only for warning message)
			if migrationStr != "" {
				changesetStr = migrationStr
				c.lggr.Warnf("--migration flag is deprecated, please use --changeset instead")
			}
	...
	cmd.Flags().StringVarP(&migrationStr, "migration", "m", "", "changeset to apply (DEPRECATED: use --changeset instead)")
	cmd.Flags().StringVarP(&inputFile, "input-file", "i", "", "YAML input file name. Not the full path, just the name (optional)")

	// Mark the migration flag as deprecated
	_ = cmd.Flags().MarkDeprecated("migration", "use --changeset instead")

	cmd.MarkFlagsOneRequired("changeset", "migration")
	cmd.MarkFlagsMutuallyExclusive("changeset", "migration")
...
func (c Commands) newDurablePipelineInputGenerate(
	domain domain.Domain,
	loadMigrationsRegistry func(envName string) (*migrations.MigrationsRegistry, error),
	loadConfigResolvers *migrations.ConfigResolverManager,
) *cobra.Command {
	var (
	...
	cmd := cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resolverByKey := make(map[string]migrations.ConfigResolver)
			for _, key := range registry.ListKeys() {
				var cfg migrations.Configurations
...
func (Commands) newDurablePipelineListBuild(domain domain.Domain, loadMigrationsRegistry func(envName string) (*migrations.MigrationsRegistry, error), loadConfigResolvers *migrations.ConfigResolverManager) *cobra.Command {
	cmd := cobra.Command{
		Use:     "list",
