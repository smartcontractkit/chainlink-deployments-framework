...
type LoadRegistryFunc func(envKey string) (*migrations.MigrationsRegistry, error)

// DecodeProposalCtxProvider is a function type that adds decoding context based on the environment.
type DecodeProposalCtxProvider func(env cldf.Environment) (mcms.ProposalContext, error)
...
func (c Commands) newMigrationRun(
	...
	decodeProposalContext func(env cldf.Environment) (mcms.ProposalContext, error),
) *cobra.Command {
	var (
...
func getChainOverrides(migration *migrations.MigrationsRegistry, migrationStr string) ([]uint64, error) {
	...
	configs, err := migration.GetConfigurations(migrationStr)
	if err != nil {
		return nil, err
	}
