...
func transactOptsWithGasOverrides(base *bind.TransactOpts, gasLimit, gasPrice, maxTxGasLimit uint64) *bind.TransactOpts {
	if base == nil {
		return nil
	}
	...
	opts := *base
	if gasLimit > 0 {
		opts.GasLimit = evm.CapGasLimit(gasLimit, maxTxGasLimit)
	}
	if gasPrice > 0 {
