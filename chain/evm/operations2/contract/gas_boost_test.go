...
func TestTransactOptsWithGasOverrides(t *testing.T) {
	...
	require.Nil(t, transactOptsWithGasOverrides(nil, 1, 1, 0, 0))
	require.Same(t, base, transactOptsWithGasOverrides(base, 0, 0, 0, 0))

	limitOnly := transactOptsWithGasOverrides(base, 500_000, 0, 0, 0)
	...
	priceOnly := transactOptsWithGasOverrides(base, 0, 30_000_000_000, 0, 0)
	...
	withGas := transactOptsWithGasOverrides(base, 500_000, 30_000_000_000, 0, 0)
	...
	buffered := transactOptsWithGasOverrides(base, 1_000_000, 0, 2500, 0)
	require.Equal(t, uint64(1_250_000), buffered.GasLimit)

	capped := transactOptsWithGasOverrides(base, 14_000_000, 0, 2500, evm.EIP7825MaxTxGasLimit)
