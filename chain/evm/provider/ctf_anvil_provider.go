...
const failedContainerCleanupTimeout = 10 * time.Second
...
func (p *CTFAnvilChainProvider) removeFailedContainer(ctx context.Context, remover containerRemover) {
	// Startup commonly fails because its context expired. Cleanup must still
	// get a chance to reach Docker, but must not outlive a fixed budget.
	cleanupCtx, cancel := context.WithTimeout(context.WithoutCancel(ctx), failedContainerCleanupTimeout)
	defer cancel()

	if p.Container != nil {
		_ = p.Container.Terminate(cleanupCtx)
		p.Container = nil

		return
	}

	_ = remover(cleanupCtx, p.config.Name)
}
