...
func NewCTFChainProvider(t *testing.T, selector uint64, config CTFChainProviderConfig) (*CTFChainProvider, error) {
	t.Helper()
...
func (p *CTFChainProvider) Initialize() error {
	...
	if err != nil {
		return fmt.Errorf("failed to generate deployer account: %w", err)
	}
	p.t.Logf("Using Aptos account: %s", deployerSigner.Address)
	...
	url, client, err := p.startContainer(chainID, deployerSigner)
	if err != nil {
		return fmt.Errorf("failed to start CTF container for chain %s: %w", chainID, err)
	}
