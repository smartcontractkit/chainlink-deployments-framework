...
func TestFindPredecessorPRs(t *testing.T) {
	...
	t.Run("success: standard scenario ", func(t *testing.T) {
		t.Parallel()
		preds, err := prFinder.FindPredecessors(t.Context(), newView, []PRNum{})
		require.NoError(t, err)
		require.Len(t, preds, 1)
		require.Equal(t, PRNum(prNumber), preds[0].Number)
	})

	t.Run("success: exclude merged PRs", func(t *testing.T) {
		t.Parallel()
		preds, err := prFinder.FindPredecessors(t.Context(), newView, []PRNum{11})
		require.NoError(t, err)
