...
func TestEnvironmentEVMRegistry_GetABIByAddress(t *testing.T) {
	...
	assert.JSONEq(t, `[{"type":"function","name":"test","inputs":[]}]`, abiStr)
}
...
func TestEnvironmentEVMRegistry_GetAllABIs(t *testing.T) {
	...
	assert.JSONEq(t, `[{"type":"function","name":"test","inputs":[]}]`, abis["test:1.0.0"])
	assert.JSONEq(t, `[{"type":"function","name":"mock","inputs":[]}]`, abis["mock:2.0.0"])
}
