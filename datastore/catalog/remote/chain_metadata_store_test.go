...
func TestCatalogChainMetadataStore_Update(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Execute update
			err := store.Update(metadata.Key(), metadata.Metadata)
...
func TestCatalogChainMetadataStore_Update_WithCustomUpdater(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err = store.Update(original.Key(), tt.incoming, datastore.WithUpdater(tt.updater))
			require.NoError(t, err)

			// Verify the update worked correctly
			retrieved, err := store.Get(original.Key())
			require.NoError(t, err)
...
func TestCatalogChainMetadataStore_Upsert_WithCustomUpdater(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := store.Upsert(key, tt.incoming, datastore.WithUpdater(tt.updater))
			...
			if tt.name == "insert_new_with_whole_metadata_merger" {
				...
				if err != nil {
					...
					err = store.Upsert(key, tt.incoming)
				}
			}
			require.NoError(t, err)
...
func TestCatalogChainMetadataStore_Update_StaleVersion(t *testing.T) {
	...
	updatedMetadata.IsTestnet = false
	err = store1.Update(first.Key(), updatedMetadata)
	require.NoError(t, err)
	...
	err = store2.Update(second.Key(), secondUpdateMetadata)
	require.NoError(t, err)

	// Verify the final state reflects the second update
	final, err := store1.Get(key)
	require.NoError(t, err)
...
func TestCatalogChainMetadataStore_Upsert(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Execute upsert
			err := store.Upsert(metadata.Key(), metadata.Metadata)
...
func TestCatalogChainMetadataStore_Upsert_StaleVersion(t *testing.T) {
	...
	err = store2.Upsert(second.Key(), upsertMetadata)
	require.NoError(t, err)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			case "filter":
				...
				results, err = store.Filter(filterFunc)
			}
